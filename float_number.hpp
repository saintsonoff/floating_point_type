...
// class FloatingPointNumber : RoundingDevice<kRounding, uint64_t> {
  public:
    using IntegerType = uint32_t;
    using MoreIntegerType = uint64_t;
    // using MajorIntegerType = uint64_t;
  ...
  private:
    ...
    MajorIntegerType ShiftLeft(uint exp_diff, MajorIntegerType mantisa) const {
        MajorIntegerType value = mantisa;
        if (!is_denormalized()) {
            value |= mant_mask() + 1;
        }

        return value << exp_diff;
    }
  ...
  public:
    ...
    FloatingPointNumber& operator-=(const FloatingPointNumber& value) {
        FloatingPointNumber negative_value = value;
